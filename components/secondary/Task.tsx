...
const Task:FC<ExtendedTaskType> = ({
  ...
  return (
    <section onClick={()=> setActive(prevState=> !prevState)} className="w-full cursor-pointer min-h-[72px] bg-brand-paleWhite rounded-[20px] flex flex-row p-10 space-x-6">
      <CustomCheckBox onClick={handleComplete} selected={completed}/>
      <div className="w-[90%]">
        <p className={`text-lg font-semibold ${completed ?"line-through": ""}`}>{title}</p>
        <p className={`${active? "h-auto opacity-100 mt-4 " : "h-0 opacity-0 mt-0"} transition-all duration-300 text-brand-slateBlue ${completed ?"line-through": ""}`}>
          {content}
        </p>
      </div>
    <div className={`flex space-x-4 ${active? "w-auto opacity-100": "w-0 opacity-0-0"}`}>
      <LeadingIcon/>
      <EditIcon/>
      <CloseIcon/>
    </div>
